...
describe("LatviaScraper", () => {
  beforeEach(() => { vi.stubGlobal("fetch", vi.fn()); });
  afterEach(() => { vi.restoreAllMocks(); vi.resetModules(); });

  it("has correct country and source", async () => {
    const { LatviaScraper } = await import("./latvia");
    const s = new LatviaScraper();
    expect(s.country).toBe("LV");
    expect(s.source).toBe("fuelo_lv");
  });
  ...
  it("delegates to fetchFueloCountry", async () => {
    ...
    expect(mockFn.mock.calls[0][0].subdomain).toBe("lv");
